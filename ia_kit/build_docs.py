# -*- coding: utf-8 -*-
"""
Render <project>_ia.json -> <project>_ia_docs.html  (프로젝트 무관 · 범용 렌더러)

IA 그래프가 단일 진실원천. 요구사항정의서/ERD/화면명세서/화면 미리보기는 모두 투영.
이 파일은 도메인 지식을 전혀 갖지 않고 JSON만 읽는다. 수정 불필요 — 복붙해서 그대로 사용.

사용법:
    python build_docs.py                 # 폴더에서 *_ia.json 자동 탐색
    python build_docs.py myproj_ia.json  # 명시 지정
"""
import json, os, sys, glob

here = os.path.dirname(os.path.abspath(__file__))
if len(sys.argv) > 1:
    json_path = sys.argv[1] if os.path.isabs(sys.argv[1]) else os.path.join(here, sys.argv[1])
else:
    cands = sorted(glob.glob(os.path.join(here, "*_ia.json")))
    if not cands:
        sys.exit("오류: *_ia.json 파일을 찾을 수 없습니다. 먼저 build_ia.py 를 실행하세요.")
    json_path = cands[0]
doc = json.load(open(json_path, encoding="utf-8"))
PROJECT_NAME = doc.get("project", {}).get("name", "IA")
PROJECT_ID = doc.get("project", {}).get("id", "project")
data_json = json.dumps(doc, ensure_ascii=False, separators=(",", ":"))

HTML = r"""<!doctype html><html lang="ko"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>__PROJECT_NAME__ · IA 문서</title>
<style>
...
.asel{font-size:11px;color:var(--mut)}.asel select{background:var(--panel2);color:var(--txt);border:1px solid var(--line);border-radius:8px;padding:5px 8px;font-size:12px;margin-left:4px}
.accsum{font-size:11px;color:var(--warn);font-weight:600}
.wb-dis{opacity:.5;filter:grayscale(.6)}
.lockbadge{margin-left:auto;font-size:10px;color:#e11d48;font-weight:700}
.robadge{font-size:9px;background:#fff3e0;color:#d97706;border-radius:8px;padding:0 5px;margin-left:4px}
.lockscr{background:#fff;border:1px dashed #e3e8f0;border-radius:13px;padding:26px 16px;margin:10px 2px;text-align:center;color:#1b2430;font-size:13px}
...
<div id="top">
  <div class="bar">
    <h1>__PROJECT_NAME__ · IA → 문서 자동 렌더링</h1>
...
function jsonValue(m,pf){const ty=typeOf(m,pf);const ex=exampleOf(m,pf);const en=(m.payload.enums||{})[pf.name];
 ...
 return ex;}
function jsonBody(m,opts){const skip=/^id$|_at$|^group$|snapshot_uuid|balance_after|reason_display|^unique$|^pk$/;
 opts=opts||{};const hide=opts.hide,mask=opts.mask;
 const obj={};let k=0;for(const f of (m.payload.fields||[])){const pf=parseField(f);if(skip.test(pf.name))continue;
   if(hide&&hide.has(pf.name))continue;            // 역할상 숨김 필드는 본문에서 제외
   if(k++>=12){obj["…"]="…";break;}
   obj[pf.name]=(mask&&mask.has(pf.name))?"••••••":jsonValue(m,pf);}
  ...
  const groups=surfaces.map(sf=>{
    ...
    const cards=screens.map(s=>{
      ...
      return `<div class="scard">
        ...
        ${(s.payload.access||[]).length?`<div class="srow"><span class="lab">접근제어</span><span class="val">${(s.payload.access||[]).map(r=>`<span class="tagk">${esc((r.roles||[]).map(x=>cleanName(byId[x])||x).join(','))}: ${esc(r.effect)} → ${esc((r.targets||[]).join(', '))}</span>`).join('')}</span></div>`:''}
      </div>`;
    }).join('');
...
let curScreen=null;
let mode='designer';
let curActor=null;              // 미리보기에서 선택한 액터(접근제어 적용 대상). null=정책 미적용
function setMode(m){mode=m;renderPreview();}
function setActor(v){curActor=v||null;renderPreview();}
// 현재 화면 + 선택 액터 기준으로 접근제어 효과 계산
function accessEffect(s){const eff={hideC:new Set(),disC:new Set(),hideT:new Set(),maskT:new Set(),roT:new Set(),blocked:false};
  if(!curActor)return eff;
  (s.payload.access||[]).forEach(r=>{ if(!(r.roles||[]).includes(curActor))return;
    (r.targets||[]).forEach(t=>{
      if(t==='self'){ if(r.effect==='hide'||r.effect==='disable')eff.blocked=true; return; }
      if(t.indexOf('comp.')===0){ (r.effect==='hide'?eff.hideC:eff.disC).add(t); return; }
      if(r.effect==='hide')eff.hideT.add(t); else if(r.effect==='mask')eff.maskT.add(t); else if(r.effect==='readonly')eff.roT.add(t);
    });
  });
  return eff;}
const _fieldHit=(set,mid,fn)=>set.has(mid+'.'+fn)||set.has(mid);
...
// UI 표면 = 화면(screen)을 1개 이상 담은 surface (문서 순서 유지)
function uiSurfaces(){return DOC.nodes.filter(n=>n.type==='surface'&&out(n.id,'contains').some(e=>byId[e.to]&&byId[e.to].type==='screen'));}
...
function renderPreview(){
  const host=document.getElementById('v-preview');
  if(!curScreen){const f=DOC.nodes.find(n=>n.type==='screen');curScreen=f?f.id:null;}
  const s=byId[curScreen]; if(!s){if(host)host.innerHTML='';return;}
  const eff=accessEffect(s);
  const sf=surfaceOf(curScreen); const sfid=sf?sf.id:'';
  const isApp = !!(sf&&sf.payload&&sf.payload.form_factor==='phone');
  const order=uiSurfaces().map(x=>x.id);
  ...
  const compBlock=c=>{if(eff.hideC.has(c.id))return'';const dis=eff.disC.has(c.id);
    const t=dis?null:compTarget(c,curScreen);const cl=t?`onclick="navTo('${t.id}')" style="cursor:pointer"`:'';
    return `<div class="wb${dis?' wb-dis':''}" ${cl}><h4>${esc(cleanName(c))}${c.payload.reusable?'<span class="reuse">재사용</span>':''}${dis?'<span class="lockbadge">⊘ 비활성</span>':(t?`<span class="goto">→ ${esc(t.payload.code)}</span>`:'')}</h4>${compFaux(c)}</div>`;};
  // mode-aware data panel (designer: 예시값 / developer: 타입·필수·enum)
  const dataPanel=(id,kind)=>{const m=byId[id];if(!m)return'';const enums=m.payload.enums||{};
    const items=(m.payload.fields||[]).map((f,i)=>({f,i,pf:parseField(f)})).filter(o=>!_fieldHit(eff.hideT,id,o.pf.name));
    const lim=mode==='developer'?9:6;
    const rows=items.map((o,vi)=>{const pf=o.pf,i=o.i,ev=enums[pf.name],ty=typeOf(m,pf),req=isReq(pf.note,pf.name);
      const hc=vi>=lim?' ff-hide':'';const masked=_fieldHit(eff.maskT,id,pf.name),ro=_fieldHit(eff.roT,id,pf.name);
      if(mode==='developer')return `<div class="ff click dev${hc}" onclick="fieldInfo('${id}',${i})"><code>${esc(pf.name)}</code><span class="ty">${ty}${req?'<span class="req">*</span>':''}${ev?' · enum('+ev.length+')':''}${masked?' · 🔒mask':''}${ro?' · ro':''}</span></div>`;
      const exv=masked?'••••••':exampleOf(m,pf,ev,ty);
      return `<div class="ff click${hc}" onclick="fieldInfo('${id}',${i})"><span>${esc(pf.name)}${ro?' <span class="robadge">readonly</span>':''}</span><em>${esc(exv)}</em></div>`;}).join('');
    const hidden=items.length-lim;
    ...
    let jsonEx='';
    if(mode==='developer'){
      const opts={hide:new Set(),mask:new Set()};
      (m.payload.fields||[]).forEach(f=>{const n=parseField(f).name;if(_fieldHit(eff.hideT,id,n))opts.hide.add(n);if(_fieldHit(eff.maskT,id,n))opts.mask.add(n);});
      if(kind==='입력'){const api=apis.find(a=>a.payload.method!=='GET'&&out(a.id,'writes').some(e=>e.to===id));
        ...
        jsonEx=`<div class="reqlbl">API 요청 예시 · ${esc(hdr)}${curActor?' · '+esc(cleanName(byId[curActor])||curActor)+' 기준':''}</div><pre class="reqjson">${esc(jsonBody(m,opts))}</pre>`;}
        ...
        jsonEx=`<div class="reqlbl">API 응답 예시 · ${esc(hdr)}${curActor?' · '+esc(cleanName(byId[curActor])||curActor)+' 기준':''}</div><pre class="reqjson">${esc(jsonBody(m,opts))}</pre>`;}
    }
    return `<div class="wb"><h4>${esc(m.name)}<span class="badge${kind==='입력'?' in':''}">${kind}</span></h4>${rows}${more}${jsonEx}</div>`;};
  let content,actions='';
  if(eff.blocked){
    content=`<div class="lockscr">🔒 이 화면은 <b>${esc(cleanName(byId[curActor])||curActor)}</b> 에게 노출되지 않습니다.<div class="kv2" style="margin-top:6px">(screen.access 의 self hide/disable 규칙)</div></div>`;
  }else{
    content=comps.filter(c=>c.payload.kind!=='navigation').map(compBlock).join('');
    [...showDM].slice(0,5).forEach(id=>content+=dataPanel(id,'표시'));
    [...inDM].slice(0,4).forEach(id=>content+=dataPanel(id,'입력'));
    actions=trans.map(t=>`<div class="wbtn" onclick="navTo('${t.id}')">${esc(cleanName(t))} <span style="opacity:.7">→ ${esc(t.payload.code)}</span></div>`).join('');
  }
  let frame;
  if(isApp){
    frame=`<div class="device phone"><div class="scr">
      ...
      <div class="tabbar">${out(sfid,'contains').map(e=>byId[e.to]).filter(n=>n&&n.type==='screen').slice(0,4).map(n=>`<a class="${n.id===curScreen?'on':''}" onclick="navTo('${n.id}')">${esc(n.payload.code)}</a>`).join('')}</div>
    </div></div>`;
  }else{
    ...
    frame=`<div class="device desktop"><div class="scr desk">
      <div class="adside"><div class="adlogo">${esc(cleanName(sf)||sf.name)}</div>${side}</div>
  ...
  host.innerHTML=`<div class="pv"><div class="pvindex">${index}</div>
    <div class="pvstage">
      <div class="pvtop"><div class="seg"><button class="${mode==='designer'?'on':''}" onclick="setMode('designer')">디자이너</button><button class="${mode==='developer'?'on':''}" onclick="setMode('developer')">개발자</button></div>
        <label class="asel">역할 <select onchange="setActor(this.value)"><option value="" ${!curActor?'selected':''}>정책 미적용(전체)</option>${DOC.nodes.filter(n=>n.type==='actor').map(a=>`<option value="${a.id}" ${a.id===curActor?'selected':''}>${esc(cleanName(a)||a.name)}</option>`).join('')}</select></label>
        ${curActor?`<span class="accsum">🔒 숨김 ${eff.hideT.size+eff.hideC.size} · ⊘ 비활성 ${eff.disC.size} · ••• 마스킹 ${eff.maskT.size}${eff.blocked?' · ⛔ 화면 차단':''}</span>`:'<span class="modedesc">역할을 고르면 접근제어(숨김/비활성/마스킹)가 적용됩니다</span>'}</div>
...
document.getElementById('exportBtn').onclick=()=>{const blob=new Blob([JSON.stringify(DOC,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=((DOC.project&&DOC.project.id)||'project')+'_ia.edited.json';a.click();};
...
# 한글 라벨/가이드/타입/예시는 모두 IA(JSON)에 저장됨 — 렌더러는 읽기만 함.
HTML = HTML.replace("__DATA__", data_json)
HTML = HTML.replace("__PROJECT_NAME__", PROJECT_NAME)
outp = os.path.join(here, f"{PROJECT_ID}_ia_docs.html")
